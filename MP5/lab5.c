 ...
 *   -u 7:  Tree with only one left node then delete the left root
 *   -u 8:  Tree with only right links and then delete root
 *
...
int main(int argc, char **argv)
{
    ...
    if (UnitNumber == 7)
    {
        // Root with only one left node and delete root
        const int ins[] = {10, 8, 26, 23, 60};
        const int del[] = {10};
        unitDriver(ins, sizeof ins/sizeof(int), del, sizeof del/sizeof(int));
    }
    if (UnitNumber == 8)
    {
        // Tree with only nodes on the right and delete root
        const int ins[] = {10, 26, 15, 34, 22};
        const int del[] = {10};
        unitDriver(ins, sizeof ins/sizeof(int), del, sizeof del/sizeof(int));
    }
