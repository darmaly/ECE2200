 ...
 *   -b
 *
 * To test using custom driver
 *   -u 1  custom driver that tests problem number 6 from the homework
 *   -u 2  custom driver that tests problem number 7 from homework as well as 
 *   		testing boundry conditions such as deleting key not in table
...
int main(int argc, char **argv)
{
    ...
    /* ---- Unit driver 1 ----*/
    if (UnitNumber == 1) {
	    //insert 5, 12, 11, and 19 into table of size 7
	    table_t *T;
	    T = table_construct(7, LINEAR);
	    printf("inserting keys 5, 12, 11, and 19 into table of size 7\n");
	    ...
	    //remove keys 5 and 12
	    printf("removing keys 5 and 12\n");
	    free(table_delete(T, 5));
	    free(table_delete(T, 12));
	    table_debug_print(T);

	    //insert keys 19 and 26
	    printf("inserting keys 19 and 26\n");
	    table_insert(T, 19, malloc(sizeof(int)));
    ...
    if (UnitNumber == 2) {
	    //insert 7, 8, 9, 10, 11, 12 into table of size 7
            table_t *T;
            T = table_construct(7, LINEAR);
            printf("inserting keys 7, 8, 9, 10, 11, and 12 into table of size 7\n");
	    ...
	    //attempting to insert into full table
	    printf("attempting to insert into a full table\n");
	    table_insert(T, 13, malloc(sizeof(int)));
	    table_debug_print(T);

            //remove keys 7, 8, 9, 10, and 11
            printf("removing keys 7, 8, 9, 10, 11\n");
	    ...
	    table_debug_print(T);
	    free(table_delete(T, 11));
            table_debug_print(T);

            //insert keys 13 and 14
            printf("inserting keys 13 and 14\n");
            table_insert(T, 13, malloc(sizeof(int)));
            table_insert(T, 14, malloc(sizeof(int)));
            table_debug_print(T);

	    //try to delete a key not in a table
	    printf("attempting to delete key 22\n");
	    table_delete(T, 22);
	    table_debug_print(T);

	    //attempting to delete from empty table
	    printf("attempting to delete from empty table\n");
	    free(table_delete(T, 13));
	    free(table_delete(T, 14));
	    free(table_delete(T, 12));
	    table_delete(T, 12);
	    table_debug_print(T);

	    //attempt to insert more items into table that is full but with deleted entries
	    printf("Filling table again and trying to add extra key when table is full\n");
	    table_insert(T, 7, malloc(sizeof(int)));
	    table_insert(T, 8, malloc(sizeof(int)));
	    table_insert(T, 9, malloc(sizeof(int)));
	    table_insert(T, 10, malloc(sizeof(int)));
	    ...
	    //inserting duplicate key when table is full
	    printf("attempting to insert 12 again\n");
	    table_insert(T, 12, malloc(sizeof(int)));
	    table_debug_print(T);
    ...
    while ((c = getopt(argc, argv, "m:a:h:i:t:s:u:erbv")) != -1)
	    ...
	    case 'u': UnitNumber = atoi(optarg);     break;
            ...
            default:
		      ...
		      printf("	-u x 	  runs custom unit driver\n");
                      exit(1);
        }
