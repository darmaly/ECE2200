/* ens_support.c
 * Deeb Armaly          <<-- many updates required including your name
 * darmaly
 * ECE 2230 Fall 2021
 * MP2
 *
...
void ens_add(linked_t *list_ptr, const int id, const int max_list_size)
{
    ...
    if(list_ptr != NULL) {
    	...
    	else if(data_ptr == NULL && linked_entries(list_ptr) == max_list_size) {
...
void ens_print_epid(linked_t *list_ptr, int id)
{
    ...
    int num_in_list = linked_entries(list_ptr);   // fixed
    int pos = 0;
...
/* This function appends the given id to the end of the exposure list
 * list_ptr: list to append to 
 * rec_count: how many times the function has been called and should be equal to rec_num
 * id: new ep_id to append to end of list
 *
 * function has no return value and should append the new id to end of 
 * exposure list 
*/
void ens_appexp(linked_t *list_ptr, int rec_count, int id) {

	//create new record to insert
	data_t *dptr = (data_t *) calloc(1,sizeof(data_t));
	dptr->ep_id = id;
	dptr->rec_num = rec_count;

	//insert record to end of list
	linked_insert(list_ptr, dptr, LLPOSITION_BACK);

}

/* This function sorts the list according to the specified sort method
 * list_ptr: list to sort
 * sort_type: can be 1,2,3,4,5 and determines the sort type to use
 *
 * has no return value but the list should be sorted and the amoutn of time it took 
 * to sort should be printed
*/
void ens_sort(linked_t *list_ptr, int sort_type) {
	clock_t start, end;
	...
	elapse_time = 1000* ((double) (end - start)) / CLOCKS_PER_SEC;
